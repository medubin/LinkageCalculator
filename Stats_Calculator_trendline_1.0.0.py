...
def write_to_file(l): # writes to a new file with a name equal to the given file (minus the 3 character extension) + the mod string
	with open("Calculations_from_average_trendlines_values.csv", "wb") as f:
		writer = csv.writer(f)
		writer.writerows(l)
...
def calculate_stats(): #Calculates
	the_list = [] 
	list_to_file = [["File", "Starting_value", "Background_value", "Distance_to_background"]]
	for files in glob.glob("*nucleotides.csv"):
		print files
		...
		print average_value
		list_to_file.append([files[0:-13],the_list[1][1],average_value, calculation_distance(the_list, average_value) ])
		the_list = []
